...
void initSeq()
{
  ...
  {
    theSeq[i] = rand() % colors + 1; // generate a random integer in the range [1, colors]
  }
}

/* display the sequence on the terminal window, using the format from the sample run in the spec */
void showSeq(int *seq) //DEBUG A BIT
{
  /* ***  COMPLETE the code here  ***  */
  ...
  for (int i = 0; i < seqlen; i++)
  {
    printf("%s ", color_names[seq[i] - 1]); // print the name of the colors
  }
  printf("\n");
}
...
{
  int *matches = (int *)malloc(sizeof(int) * 2);
  matches[0] = 0;
  matches[1] = 0;

  int array1[3] = {0}; // Marks exact matches in seq1
  int array2[3] = {0}; // Marks exact matches in seq2
  ...
  for (int i = 0; i < 3; i++)
  {
    if (!array2[i]) // If the color in seq2 at position i has not been matched yet
    {
      for (int j = 0; j < 3; j++)
      {
        if (!array1[j] && seq1[j] == seq2[i]) // If the color in seq1 at position j has not been matched yet and it matches the color in seq2 at position i
        {
          matches[1]++;
          array1[j] = 1;
          break;
...
{
  /* ***  COMPLETE the code here  ***  */
  // assign exact and approximate matches to variables
  int exact_matches = code[0];
  int approx_matches = code[1];
...
int readNum(int max) //CHANGE - FILL
{
  /* ***  COMPLETE the code here  ***  */
}
...
void initITimer(uint64_t timeout)
{
  ...
  printf("configuring a timer with a delay of %d micro-seconds ...\n", DELAY);
...
void blinkN(uint32_t *gpio, int led, int c)
{
  ...
  for (int i = 0; i < c; i++)
  {
    digitalWrite(gpio, led, HIGH); // Turn the LED on
    delay(250);                    // Wait for 500ms
    digitalWrite(gpio, led, LOW);  // Turn the LED off
    delay(250);                    // Wait for 500ms
  }
}
...
{ // this is just a suggestion of some variable that you may want to use
  ...
  int pinGREENLED = GREENLED, pinREDLED = REDLED, pinButton = BUTTON;
  ...
  // controlling LED pin 05
  fSel = 0;   // GPIO 05 lives in register 0 (GPFSEL0)
  shift = 15; // GPIO 15 sits in slot 5 of register 0, thus shift by 5*3 (3 bits per pin)
  // C version of setting LED
  *(gpio + 0) = (*(gpio + 0) & ~(7 << 15)) | (1 << 15); // Sets bits to one = output

  // controlling LED pin 13
  fSel = 1;  // GPIO 13 lives in register 1 (GPFSEL1)
  shift = 9; // GPIO 13 sits in slot 3 of register 1, thus shift by 3*3 (3 bits per pin)
  // C version of setting LED
  *(gpio + 1) = (*(gpio + 1) & ~(7 << 9)) | (1 << 9); // Sets bits to one = output

  // controlling button pin 19
  fSel = 1;   // GPIO 19 lives in register 1 (GPFSEL1)
  shift = 27; // GPIO 19 sits in slot 9 of register 1, thus shift by 9*3 (3 bits per pin)
  // C version of mode input for button
  *(gpio + 1) = *(gpio + 1) & ~(7 << 27); // Sets bits to one = output
  ...
  printf("Sequence of 3 colours represented by 1, 2 and 3 respectively for R, G and B.\n");
  showSeq(theSeq);

  lcdPosition(lcd, 0, 0);
  lcdPuts(lcd, "PRESS ENTER!");
  lcdPosition(lcd, 0, 1);
  lcdPuts(lcd, "START GUESSING!");
  ...
  {
    fprintf(stderr, "Press 1 for R, 2 for G and 3 for B!\n");
    attempts++;

    // Display the attempt number on the LCD
    lcdPosition(lcd, 0, 0);
    sprintf(str1, "Attempt: %d", attempts);
    lcdPuts(lcd, str1);

    buttonPressed = 0; // initialize number of buttonPressed to 0'

    // Check for button presses and count them
    for (int i = 0; i < seqlen; i++)
    {
      delay(500);
      //  Set timer of 5 seconds between each input of button
      initITimer(5);
      // Loop until the timer to button input runs off
      while (!timed_out)
      {
        ...
        if (readButton(gpio, pinButton))
        {
          if (buttonPressed < 3) {
          buttonPressed++;
          fprintf(stderr, "Button has been pressed %d time(s)...\n", buttonPressed);
        }
      }
        delay(200); // Delay before iterating
      }

      attSeq[i] = buttonPressed; // Assign guessed number to attSeq

      // Display the current guess on the LCD
      lcdPosition(lcd, 0, 1);
      lcdPuts(lcd, "GUESS: ");
      for (int j = 0; j <= i; j++)
      {
        // Use attSeq[j] as an index to colors array
        switch (attSeq[j])
        {
        case 1:
          lcdPuts(lcd, " R");
      ...
      // Red Led Blink (1) - Separator
      blinkN(gpio, pinREDLED, 1); 
      delay(500);
      timed_out = 0;
      blinkN(gpio, pinGREENLED, buttonPressed); // Green LED Blink - Number of Button Pressed
      buttonPressed = 0;                        // Reset buttonPressed
    }
    
    lcdClear(lcd);

    delay(500);
    blinkN(gpio, pinREDLED, 2); // Red Led Blink (2) - Separator
    delay(500);

    // Check the sequence matches and put it in result
    ...
    // Green Led Blink - Number of Exact
    blinkN(gpio, pinGREENLED, exact);

    // Print Exact on LCD
    sprintf(str1, "%d Exact", exact);
    lcdPosition(lcd, 0, 0);
    lcdPuts(lcd, str1);
    ...
    // Green Led Blink - Number of Approx
    blinkN(gpio, pinGREENLED, contained);

    // Print Approx on LCD
    sprintf(str1, "%d Approximate", contained);
    lcdPosition(lcd, 0, 1);
    lcdPuts(lcd, str1);
    delay(500);

    // Red Led Blink (3) - New Round
    blinkN(gpio, pinREDLED, 3);
  ...
  if (found)
  {
    ...
    lcdPosition(lcd, 0, 0);
    lcdPuts(lcd, "SUCCESS!");
    sprintf(str1, "In %d attempt(s)", attempts);
    lcdPosition(lcd, 0, 1);
    lcdPuts(lcd, str1);

    // Turn on the red LED
    digitalWrite(gpio, pinREDLED, HIGH);

    // Make the green LED blink three times
    blinkN(gpio, pinGREENLED, 3);

    // Turn off both the red LEDs
    digitalWrite(gpio, pinREDLED, LOW);
    digitalWrite(gpio, pinGREENLED, LOW);
  ...
  else
  {
    ...
    lcdClear(lcd);
    lcdPosition(lcd, 0, 0);
    lcdPuts(lcd, "Better Luck");
    lcdPosition(lcd, 0, 1);
    lcdPuts(lcd, "Next Time!");
    delay(200);
    lcdClear(lcd);
